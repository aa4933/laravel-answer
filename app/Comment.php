...
class Comment extends Model
{
    ...
    public function add()
    {
        ...
        /*是否有评论内容*/
        if (!rq('content'))
            return ['status' => 0, 'msg' => '没有评论内容'];
        /*不能同时不存在问题id与回答id*/
        if (!rq('question_id') && !rq('answer_id'))
            return ['status' => 0, 'msg' => '不存在问题id与回答id'];
        /*不能同时存在问题id与回答id*/
        if (!rq('question_id') && !rq('answer_id'))
            return ['status' => 0, 'msg' => '不存在问题id与回答id'];
        /*问题id*/
        if (rq('question_id')) {
            ...
            if (!$question)
                return ['status' => 0, 'msg' => '不存在问题'];
            $this->question_id = rq('question_id');
        } /*回答id*/
        else {
            $answer = answer_ins()->find(rq('answer_id'));
            if (!$answer)
                return ['status' => 0, 'msg' => '不存在回答'];
        ...
        if (rq('reply_to')) {
            ...
            if (!$reply)
                return ['status' => 0, 'msg' => '不存在这个评论'];
            if ($reply->user_id == session('user_id'))
                return ['status' => 0, 'msg' => '不能回复自己'];
    ...
    public function look()
    {
        ...
        if (!rq('question_id') && !rq('answer_id'))
            return ['status' => 0, 'msg' => '必须存在问题或者回答的信息'];
        /*有关问题的评论*/
        if (rq('question_id')) {
            $question = question_ins()->find(rq('question_id'));
            if (!$question)
                return ['status' => 0, 'msg' => '不存在这个问题'];
    ...
    public function del()
    {
        /*用户是否登录*/
        if (!user_ins()->is_login())
            return ['status' => 0, 'msg' => '用户请登录'];
        /*是否传入ID*/
        if (!rq('id'))
            return ['status' => 0, 'msg' => '请传入需要删除的信息'];
        /*传入的ID是否有效*/
        $comment = $this->find(rq('id'));
        if (!$comment)
            return ['status' => 0, 'msg' => '传入的信息无效'];
        /*用户是否有权限删除*/
        if ($comment->user_id != session('user_id'))
            return ['status' => 0, 'msg' => '您没有权限删除'];
        /*删除相关的评论*/
